...
class GradedTestCase(unittest.TestCase):
  ...
  def run_with_solution_if_possible(self, runmodel, func):
    if use_solution:
      return func(solution)
    else:
      return func(runmodel)

  def compare_with_solution_or_wait(self, runmodel, func_name, comp):
    start = time.perf_counter()
    ans2 = comp(getattr(runmodel, func_name))
...
if __name__ == '__main__':
  ...
  project.addTest(unittest.defaultTestLoader.discover('.', pattern='tester.py'))
  CourseTestRunner(gradescope=True).run(project)