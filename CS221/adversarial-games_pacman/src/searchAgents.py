...
when running pacman.py.  Arguments can be passed to the agent
...
class CornersProblem(search.SearchProblem):
  ...
  def getStartState(self):
    "Returns the start state (in a state space, not the full Pacman state space)"
    return self.startState
...
def cornersHeuristic(state, problem):
  ...
  A heuristic for the CornersProblem that you defined.
    ...
    state:   The current search state 
             (a data structure you chose in a search problem)
...
def foodHeuristic(state, problem):
  """
  Heuristic for the FoodSearchProblem goes here.
  """
...
class DistanceCalculator:
  ...
  and call the getDistance function as necessary. 
  """
  def __init__(self, layout, default = 10000):
    """
    Initialize with DistanceCalculator(layout).
    """
...
class ApproximateSearchAgent(Agent):
  "Implement a contest entry here. "
...
def mazeDistance(point1, point2, gameState):
  ...
  This might be a useful helper function for ApproximateSearchAgent.
  """
  x1, y1 = point1
  x2, y2 = point2
  walls = gameState.getWalls()
