...
code to run a game.  This file is divided into three sections:
  ...
  (i)  The interface to the pacman world
  ...
  (iii) Framework to start a game:
          ...
          to set up the game, then starting up a new game, along with
          linking in all the external parts (agent functions, graphics).

To play the first game, type 'python pacman.py' from the command line.
...
def loadAgent(pacman, nographics):
  ...
  for moduleDir in pythonPathDirs:
    ...
    moduleNames = [f for f in os.listdir(moduleDir) if f.endswith('gents.py') or f=='runmodel.py']
...
def replayGame( layout, actions, display ):
    import runmodel, ghostAgents
    rules = ClassicGameRules()
    # If replaying, change the agent from ExpectimaxAgent to whatever agent with which you want to play
    agents = [runmodel.ExpectimaxAgent()] + [ghostAgents.RandomGhost(i+1) for i in range(layout.getNumGhosts())]
