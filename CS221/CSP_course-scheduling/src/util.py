...
def get_or_variable(csp, name, variables, value):
    ...
    True iff at least one of the |variables| is assigned to |value|. 
...
def extract_course_scheduling_solution(profile, assign):
    """
    Given an project returned from the CSP solver, reconstruct the plan. It
    ...
    @param assign: An project of variables as generated by the CSP
        solver.

    @return result: return a list of (quarter, courseId, units) tuples according
        to the solution sorted in chronological of the quarters provided.
    """
    result = []
