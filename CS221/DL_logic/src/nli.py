...
argparser.add_argument('--grammar', help='Which grammar to use', default='model')
...
class NaturalLanguageInterface:
    def __init__(self):
        ...
        # Setup model
        if opts.grammar == 'model':
            self.rules = nlparser.createBaseEnglishGrammar()
            for createRule in [model.createRule1, model.createRule2, model.createRule3]:
                try:
            ...
            raise Exception('Unknown grammar: either model or standard')
        if opts.train:
