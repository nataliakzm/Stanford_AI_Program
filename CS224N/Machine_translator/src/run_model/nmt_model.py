...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        src_padded_embedding = self.model_embeddings.source(source_padded) #  (src_len, b, e)
        
        packed_input = nn.utils.rnn.pack_padded_sequence(src_padded_embedding, source_lengths)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(y_embedding, 1):
            # print(Y_t)
            # print(Y_t.shape)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, -1)), -1) # (b, src_len)  

        # Set e_t to -inf where enc_masks has 1
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.bool(), -float('inf'))
    
        alpha_t = nn.Softmax(dim=-1)(e_t) # (b, src_len)
