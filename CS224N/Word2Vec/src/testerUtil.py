  ...
  def __call__(self, func):
    ...
    def wrapper(*args, **kwargs):
      ...
      if self.is_hidden and not use_solution:
        ...
        # Do this at the end so that the model's code is run and the elapsed time is calculated.
...
def blockPrint():
  sys.stdout = open(os.devnull, 'w') # Disable
...
class GradedTestCase(unittest.TestCase):
  ...
  def run_with_solution_if_possible(self, model, func):
    if use_solution:
      return func(solution)
    else:
      return func(model)

  def compare_with_solution_or_wait(self, model, func_name, comp):
    start = time.perf_counter()
    ans2 = comp(getattr(model, func_name))
...
class ModelTestResult(unittest.TestResult):
  ...
  def stopTestRun(self):
    if not use_solution:
      ...
      self.stream.write('Points for these parts not assigned by the tester unless the solution is present (indicated by "???").\n')
